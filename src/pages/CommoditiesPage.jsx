...
// Commodities module (placeholder). Uses the same engine/UI, but stores trades separately.
export default function CommoditiesPage() {
  const [trades, setTrades] = useLocalStorage("commodity_trades", []);
  ...
  const addTrade = (trade) => {
    const taggedTrade = { ...trade, asset: "COMMODITY" };
    ...
    <div className={`app-container ${flash ? `flash-${flash}` : ""}`}>
      <div className="container-fluid py-3">
        <div className="d-flex justify-content-between border-bottom align-items-center pb-2 mb-3">
          <h5 className="mb-0">🧱 Commodities Trading</h5>
        </div>
